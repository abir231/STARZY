...
$list = $commandeController->getCommandes(); // ✅ $list avec le "$"
$totalHT = 0;
$quantiteTotale = 0;

foreach ($list as $cmd) {
    $prod = $produit->getProduitById($cmd['id_produit']);
    $quantite = intval($cmd['montant'] / $prod['prix']); // ← quantité réelle commandée
    $quantiteTotale += $quantite;
    $totalHT += $cmd['montant'];
}

$remise = $totalHT * 0.10; // 10% de remise
$totalTTC = $totalHT - $remise;
...
<head>
    ...
    <style>
        .text-muted {
    color:rgb(123, 95, 195) !important; /* Blanc clair */
}
...
">
    ...
    <a class="nav-link active cosmic-link" href="#">
        ...
        <span class="cosmic-badge"><?= $quantiteTotale ?></span>
    </a>
</li>
                ...
                <li class="nav-item">
            <button onclick="exporterPDF()" class="btn btn-sm btn-outline-light ms-2" style="margin-top: 6px;">
...
@keyframes fadeIn {
            ...
            <div class="col-lg-8 mb-4">
                <div class="cart-card p-4 h-100">
                    <div class="table-responsive">
                        <table  id="monTableau" class="table table-cart">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">Statut</th> <!-- Nouvelle colonne "Statut" -->
    ...
    <td>
              <small class="text-muted"  ><?= $cmd['statut']; ?></small> <!-- Affichage du statut de la commande -->
            </td>                         <td>
...
<div id="address-form" class="mt-4" style="display: none;">
    ...
    <div class="floating-cart-icon">
        ...
        <div class="cart-badge"><?= $quantiteTotale ?></div>
        </div>
...
<script>
    function exporterPDF() {
        ...
        rows.forEach(row => {
            ...
            const produit = cells[1].innerText.trim();
            const prix = cells[2].innerText.trim();
            const quantite = cells[3].querySelector("input")?.value || "";
            const dispo = cells[4].innerText.trim();
            const montant = cells[5].innerText.trim();
            const statut = cells[6].innerText.trim();

            data.push([imageData, produit, prix, quantite, dispo, montant, statut]);
        });
        ...
        const columns = [
            ...
            { header: 'Disponibilité', dataKey: 'dispo' },
        ...
        doc.autoTable({
            ...
            didDrawCell: function (data) {
                if (data.column.index === 0 && data.cell.raw.image) {
                    doc.addImage(data.cell.raw.image, 'JPEG',
                        data.cell.x + 5,
                        data.cell.y + 5,
                        60,
                        60);
        ...
        const sousTotal = "<?= number_format($totalHT, 2, ',', ' ') ?> €";
        const livraison = "Gratuite";
        const remise = "-<?= number_format($remise, 2, ',', ' ') ?> €";
        const totalTTC = "<?= number_format($totalTTC, 2, ',', ' ') ?> €";

        doc.setFontSize(16);
        doc.text("Résumé de la commande", 40, doc.lastAutoTable.finalY + 30);

        const startY = doc.lastAutoTable.finalY + 50;
        const lineHeight = 20;
        doc.setFontSize(12);

        doc.text("Sous-total :", 40, startY);
        doc.text(sousTotal, 300, startY);

        doc.text("Livraison :", 40, startY + lineHeight);
        doc.text(livraison, 300, startY + lineHeight);

        doc.text("Remise (10%) :", 40, startY + lineHeight * 2);
        doc.text(remise, 300, startY + lineHeight * 2);

        doc.line(40, startY + lineHeight * 3 + 5, 500, startY + lineHeight * 3 + 5);

        doc.setFontSize(14);
        doc.setFont(undefined, 'bold');
        doc.text("Total TTC :", 40, startY + lineHeight * 4);
        doc.text(totalTTC, 300, startY + lineHeight * 4);
...
<script>
    function calculerTotalTTC() {
        let total = 0;
        document.querySelectorAll('.montant').forEach(span => {
            let texte = span.textContent.trim().replace('€', '').replace(',', '.');
            let montant = parseFloat(texte);
            if (!isNaN(montant)) {
                total += montant;
            }
        });
        // Mettre à jour l’élément avec id "totalTTC"
        document.getElementById('totalTTC').textContent = total.toFixed(2).replace('.', ',') + ' €';
    }

    window.addEventListener('DOMContentLoaded', calculerTotalTTC);
</script>
