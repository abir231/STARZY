...
require_once(__DIR__ .'/../controller/fichiercontroller.php');
require_once(__DIR__ .'/../controller/cmdcontroller.php');
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $cmdcontroller->deleteCommandebyidprod(($id));
...
$produits = $produitController->getProduits(); // Par défaut, afficher tout

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $nom = isset($_POST['productNameFilter']) ? $_POST['productNameFilter'] : '';
    $categorie = isset($_POST['categoryFilter']) ? $_POST['categoryFilter'] : '';
    $tri = '';

    if (!empty($_POST['tri'])) {
        // Traduire les valeurs de tri venant du formulaire
        if ($_POST['tri'] == 'alphabetical') {
            $tri = 'nom';
        } elseif ($_POST['tri'] == 'availability') {
            $tri = 'disponibite';
        }
    }

    if (!empty($nom)) {
        // Rechercher par nom et trier si besoin
        $produits = $produitController->rechercherEtTrierProduits($nom, $tri ?: null);
    } elseif (!empty($categorie)) {
        // Rechercher par catégorie
        $produits = $produitController->rechercherProduitsParCategorie($categorie);
    } elseif (!empty($tri)) {
        // Seulement trier sans filtre sur le nom
        $produits = $produitController->rechercherEtTrierProduits('', $tri);
    }
}

$produitsParPage = 2; // ✅ Affiche 2 produits par page
$pageActuelle = isset($_GET['page']) ? (int)$_GET['page'] : 1;

$offset = ($pageActuelle - 1) * $produitsParPage;

// ✅ Découpe le tableau pour la page actuelle
$produitsPage = array_slice($produits, $offset, $produitsParPage);

// ✅ Calcule le nombre total de pages
$nombreTotalPages = ceil(count($produits) / $produitsParPage);
...
.chart-container {
            ...
            margin: 0 auto;
            max-width: 400px;
			background-color: rgb(88, 100, 234);
        }
...
<div class="cosmic-search-container" style="max-width: 700px; margin: 30px auto; font-family: 'Arial', sans-serif;">
<form action="indiv.php" method="POST">  
    <div style="display: flex; gap: 10px; margin-bottom: 15px;">
        <div class="cosmic-search-box" style="flex: 1; display: flex; align-items: center; background: linear-gradient(135deg, #1a2a3a, #2c3e50); padding: 12px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.3);">
            <i class="fas fa-telescope cosmic-search-icon" style="color: #ecf0f1; margin: 0 15px; font-size: 20px;"></i>
            <input type="text" id="productSearch" name="productNameFilter" placeholder="Rechercher un produit..." style="flex: 1; padding: 12px; border: none; border-radius: 8px; font-size: 16px; background-color: rgba(255, 255, 255, 0.1); color: #ecf0f1; outline: none;">
            <button type="submit" name="searchButton" id="searchButton" style="background: linear-gradient(135deg, #3498db, #2c3e50); border: none; color: white; padding: 12px 20px; margin-left: 10px; border-radius: 8px; cursor: pointer; font-weight: bold;">
                <i class="fas fa-search"></i> Chercher
            </button>
        </div>

        <div class="btn-group" style="align-self: center;">
            <input type="hidden" name="tri" id="tri"> <!-- CHAMP CACHÉ pour le tri -->
            ...
            <ul class="dropdown-menu" 
                style="background: #2c3e50; border: 1px solid #3498db;"
                aria-labelledby="dropdownMenuButton">
                <li><a href="#" class="dropdown-item" onclick="setTri('alphabetical')">Ordre alphabétique</a></li>
                <li><a href="#" class="dropdown-item" onclick="setTri('availability')">Par disponibilité</a></li>
            </ul>
        </div>
    </div>


<!-- Script pour changer la valeur du tri -->
<script>
function setTri(value) {
    document.getElementById('tri').value = value; // mettre la valeur dans le champ caché
    document.forms[0].submit(); // envoyer le formulaire
}
</script>


        <!-- Filtres de recherche (Catégorie + Produit) -->
        <div class="search-filters" style="display: flex; gap: 10px; margin-top: 15px;">
            <!-- Filtre par Catégorie -->
            <select name="categoryFilter" id="categoryFilter" style="flex: 1; padding: 10px; border-radius: 8px; background: #34495e; color: #ecf0f1; border: none; outline: none; cursor: pointer;">
                <option value="">Toutes les catégories</option>
                <option value="etudiants">étudiants</option>
                <option value="universitaire">universitaires</option>
                <option value="enfant">enfant</option>
                <option value="chercheurs">chercheurs</option>
            </select>

            <!-- Filtre par Nom de Produit -->
            <input type="text" name="productNameFilter" id="productNameFilter" placeholder="Filtrer par nom..." style="flex: 2; padding: 10px; border-radius: 8px; background: #34495e; color: #ecf0f1; border: none; outline: none;">
        </div>

        <!-- Suggestions de recherche (optionnel) -->
        <div class="search-suggestions" style="margin-top: 10px; background: #2c3e50; border-radius: 8px; padding: 10px; color: #ecf0f1; display: none;">
            <p style="margin: 0 0 8px 0; font-size: 14px;">Suggestions :</p>
            <div style="display: flex; flex-wrap: wrap; gap: 5px;">
                <span class="suggestion-tag" style="display: inline-block; background: #3498db; padding: 4px 8px; border-radius: 4px; cursor: pointer;">iPhone 15</span>
                <span class="suggestion-tag" style="display: inline-block; background: #3498db; padding: 4px 8px; border-radius: 4px; cursor: pointer;">T-shirt SpaceX</span>
                <span class="suggestion-tag" style="display: inline-block; background: #3498db; padding: 4px 8px; border-radius: 4px; cursor: pointer;">Livre Astrophysique</span>
            </div>
        </div>
    </form>

    <!-- Intégration nécessaire pour le fonctionnement -->
...
<!-- Script pour la recherche combinée -->
...
<!-- Animation au focus -->
<script>
document.querySelector('.cosmic-search-input').addEventListener('focus', function() {
    this.parentElement.style.boxShadow = '0 4px 20px rgba(52, 152, 219, 0.6)';
    this.style.backgroundColor = 'rgba(255, 255, 255, 0.15)';
    document.querySelector('.search-suggestions').style.display = 'block';
});

document.querySelector('.cosmic-search-input').addEventListener('blur', function() {
    this.parentElement.style.boxShadow = '0 4px 15px rgba(0, 0, 0, 0.3)';
    this.style.backgroundColor = 'rgba(255, 255, 255, 0.1)';
    // Cache les suggestions après un délai pour permettre le clic
    setTimeout(() => {
...
<!-- Ajoute cette ligne pour inclure Font Awesome si tu ne l'as pas encore dans ton projet -->
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
		...
		<div id="sidebar" class="sidebar js-sidebar">
          ...
          <span class="align-middle">STARZY</span>
              ...
              <i class="align-middle" data-feather="sliders"></i> <span class="align-middle">Tableau de bord</span>
				...
				<a class="sidebar-link" href=" ">
                    <i class="align-middle" data-feather="produits"></i> <span class="align-middle">Gestion des activités de patrimoine</span>
                  </a>
                ...
                <!-- Nouveau lien pour la gestion des ressources -->
                <a class="sidebar-link" href="liste.php">
                    <i class="align-middle" data-feather="book"></i> <span class="align-middle">Gestion des ressources</span>
                </a>
			</li>
			...
			<a href="../view/users.php" class="sidebar-link">
    ...
    <?php foreach ($produitsPage as $product):
		if($product['categorie']=='universitaire')$universitaire++;
		if($product['categorie']=='etudiants')$etudiant++;
		if($product['categorie']=='chercheurs')$chercheur++;
		if($product['categorie']=='enfant')$enfant++;
		$nbrtotal++;
...
<div class="custom-pagination">
    <ul class="pagination">
        ...
        <?php if ($pageActuelle > 1): ?>
            <li class="page-item">
                <a class="page-link" href="?page=<?= $pageActuelle - 1 ?>">Previous</a>
            </li>
        <?php endif; ?>
        ...
        <!-- Lien "Next" -->
        <?php if ($pageActuelle < $nombreTotalPages): ?>
            <li class="page-item">
                <a class="page-link" href="?page=<?= $pageActuelle + 1 ?>">Next</a>
            </li>
        <?php endif; ?>
    </ul>
</div>
...
<script>
    // Données pour la roue de statistiques
    const statData = [
        { label: "chercheurs", value: <?php echo json_encode(($chercheur/$nbrtotal)*100); ?>, color: "rgb(21, 32, 54)" },
        { label: "enfant", value: <?php echo json_encode(($enfant/$nbrtotal)*100); ?>, color: "rgb(22, 28, 100)" },
        { label: "universitaire", value: <?php echo json_encode(($universitaire/$nbrtotal)*100); ?>, color: "#FFFFFF" },
        { label: "etudiant", value: <?php echo json_encode(($etudiant/$nbrtotal)*100); ?>, color: "rgb(51, 138, 197)" }
    ];
    ...
    // Variable pour éviter de redessiner plusieurs fois
    let isChartRendered = false;

    // Fonction pour vérifier le défilement et afficher le graphique
    function handleScroll() {
        if (isChartRendered) return; // Ne rien faire si le graphique est déjà rendu

        // Obtenir la position du conteneur par rapport au haut de la page
        const containerTop = chartContainer.getBoundingClientRect().top + window.scrollY;
        const scrollPosition = window.scrollY + window.innerHeight;

        // Déclencher lorsque le conteneur est visible (ou ajuster le seuil selon vos besoins)
    ...
    // Gestionnaire d'événement pour le clic sur le bouton
        ...
        // Dessiner la roue de statistiques
        drawPieChart();
...
// Optionnel : Supprimer l'écouteur pour optimiser les performances
window.removeEventListener('scroll', handleScroll);
	
	}}
	window.addEventListener('scroll', handleScroll);

// Vérifier immédiatement au chargement (au cas où le conteneur est déjà visible)
handleScroll();
		
    

    // Fonction pour dessiner la roue de statistiques
    // Remplacez votre fonction drawPieChart actuelle par celle-ci pour ajouter une animation
...
function drawPieChart() {
    ...
    const total = statData.reduce((sum, item) => sum + item.value, 0);
    
    // Variables pour l'animation
    let animationProgress = 0;
    const animationDuration = 1000; // durée en millisecondes
    ...
    function animate(currentTime) {
        ...
        for (const item of statData) {
            // Calculer l'angle de fin pour ce segment avec animation
            const fullSliceAngle = (item.value / total) * 2 * Math.PI;
            ...
            // Ajouter le texte du pourcentage seulement quand l'animation est presque terminée
            if (animationProgress > 0.9) {
                const middleAngle = startAngle + (fullSliceAngle / 2);
                ...
                const textOpacity = (animationProgress - 0.9) * 10; // 0 à 1 sur les 10% finaux
                
                ctx.fillStyle = item.color === "#FFFFFF" ? 
                    `rgba(51, 51, 51, ${textOpacity})` : 
            ...
            startAngle = startAngle + fullSliceAngle * animationProgress;
        }
        ...
        if (animationProgress < 1) {
            ...
            // Animation terminée, redessiner une dernière fois pour s'assurer que tout est affiché correctement
            drawFinalChart();
        }
    }
    ...
    // Fonction pour dessiner le graphique final (sans animation)
    ...
    function createLegend() {
        // Vider la légende existante
        legend.innerHTML = '';

        // Créer un élément pour chaque entrée de données
        for (const item of statData) {
